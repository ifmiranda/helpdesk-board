...
useEffect(() => {
    function scheduleNext() {
        ...
        timerRef.current = setTimeout(() => {
            ...
            const copy = [...prev];
            copy[i] = {...chosen, status: nextStatus, priority: nextPriority, updatedAt: new Date().toISOString() };
            return copy; 
        });
            scheduleNext(); 
        }, delay); 
...
const visibleTickets = useMemo(() => {
    ...
    return tickets.filter(t => {
        const statusOk = filter.status === 'All' || t.status === filters.status;
...
const removeFromQueue = id => setQueue(q => { const c = {...q}; delete c[id]; return c;}); 
...
return( 
    <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <MyQueueSummary tickets={tickets} queue={queue} onRemove={removeFromQueue} onClear={clearQueue}/> 
        </div>
